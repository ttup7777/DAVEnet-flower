...
def calc_recalls(image_outputs, audio_outputs, nframes, simtype='MISA'):
    ...
    n = S.size(0)
    A2I_scores, A2I_ind = S.topk(10, 0) #rows
    I2A_scores, I2A_ind = S.topk(10, 1) #cols
    A_r1 = AverageMeter() #initial to 0
    A_r5 = AverageMeter()
    A_r10 = AverageMeter()
    I_r1 = AverageMeter()
    I_r5 = AverageMeter()
...
def sampled_margin_rank_loss(image_outputs, audio_outputs, nframes, margin=1., simtype='MISA'): #loss function
    """
    ...
    for i in range(n):
        ...
        anchorsim = matchmapSim(computeMatchmap(image_outputs[i], audio_outputs[i][:, 0:nF]), simtype) # anchor difference
