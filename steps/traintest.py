...
def train(audio_model, image_model, train_loader, test_loader, args):
    ...
    #fi = open("./Intermediate_result.txt", 'a')
    def _save_progress():
    ...
    while epoch<=100:
        ...
        audio_model.train()
        image_model.train()
        for i, (image_input, audio_input, nframes, Imlabel, Audiolabel) in enumerate(train_loader): #call train data
            ...
            loss = sampled_margin_rank_loss(image_output, audio_output,
                nframes,Imlabel, Audiolabel, margin=args.margin, simtype=args.simtype)
