...
class SpeechDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        self.embeddings_num = cfg.SPEECH.CAPTIONS_PER_IMAGE    #value is 10  一张图由十个不同的句子描述
        ...
        if data_dir.find('birds') != -1:   #find 函数找不到时返回-1 对于bird采用bbox
            ...
            self.bbox = None
        if self.split == 'val' or self.split =='sne':
            split = 'test' 
        ...
        # if split == 'val':
        #     self.number_example = len(self.filenames) * self.embeddings_num
        # else:
        if self.split == 'test':
            self.number_example = len(self.filenames) * self.embeddings_num
        else:
    ...
    def load_filenames(self, data_dir, split):      
        ...
        if split != 'gan':
            ...
            filepath = '%s/%s/seleted_filenames_for_visual_gan.pickle' % (data_dir, 'test')
    ...
    def __getitem__(self, index):
        ...
        if self.split=='test':
            audio_index = index
            index = int(index/self.embeddings_num)
        ...
        if cfg.SPEECH.style == 'WAV':
            ...
            if self.split=='train':
                audio_ix = random.randint(0, self.embeddings_num)
            elif self.split=='val' or self.split == 'sne' or self.split == 'gan':
                audio_ix = 0
            else:
                audio_ix = audio_index % self.embeddings_num
