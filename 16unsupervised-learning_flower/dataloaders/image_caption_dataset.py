...
class ImageCaptionDataset(Dataset):
    def __init__(self, pickle_file, audio_conf=None, image_conf=None, val_option  = False):
        """
        ...
        self.image_base_path = "../data/Oxford102/images/"
        self.audio_base_path = "../data/Oxford102/audio/"

        self.image_label_path = "../flowers/image_labels.json"
        self.audio_label_path = "../flowers/audio_labels.json"
        ...
        if not val_option:
            ...
            with open("../data/Oxford102/test/audio_filenames.pickle",'rb') as f:
                filename = pickle.load(f)
        ...
        crop_size = self.image_conf.get('crop_size', 224) #dictionary .get() function 
                                                          # None if the key is not found and value is not specified.
                                                          # value if the key is not found and value is specified;
        center_crop = self.image_conf.get('center_crop', False)

        if center_crop: # center_crop for validation
            self.image_resize_and_crop = transforms.Compose(
                [transforms.Resize(256), transforms.CenterCrop(224), transforms.ToTensor()])
        else: # False; for training
            self.image_resize_and_crop = transforms.Compose(
    ...
    def _LoadAudio(self, path):
        ...
        target_length = self.audio_conf.get('target_length2', 2048)
        ...
        if p > 0: #smaller than 2048
            logspec = np.pad(logspec, ((0,0),(0,p)), 'constant', constant_values=(padval,padval))
            n_frames = target_length
        elif p < 0: #larger than 2048
            logspec = logspec[:,0:target_length]
            n_frames = target_length
    ...
    def _LoadLabels(self,labspath):
        with open(labspath, 'r') as fp:
    ...
    def __getitem__(self, index):
        ...
        dataum = self.data[index]
        Imlabel_json = self._LoadLabels(self.image_label_path)
        Audiolabel_json = self._LoadLabels(self.audio_label_path)
        if not self.val_option:
            imgfilename = dataum+'.jpg'
            img_name = dataum.split('/')
            n_randaudio = np.random.randint(0,9)
            wavfilename = dataum+'/'+ img_name[1]+'_'+ str(n_randaudio) +'.wav'

            
            Imlabel = Imlabel_json[img_name[1]]
            Audiolabel = Audiolabel_json[img_name[1]+'_'+ str(n_randaudio)]

            wavpath = os.path.join(self.audio_base_path, wavfilename)
            imgpath = os.path.join(self.image_base_path, imgfilename)
        
            audio = self._LoadAudio(wavpath)
            image = self._LoadImage(imgpath)
            
        elif self.val_option:
            wavfilename = dataum+'.wav'
            imgfilename = dataum.split('/')[0]+"/"+dataum.split('/')[1]+'.jpg'

            Imlabel = Imlabel_json[dataum.split('/')[1]]
            Audiolabel = Audiolabel_json[dataum.split('/')[2]]
        ...
        return image, audio,  Imlabel, Audiolabel
