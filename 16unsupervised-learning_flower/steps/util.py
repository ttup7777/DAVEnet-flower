...
def calc_rank_k(image_outputs, audio_outputs, Imlabel, Audiolabel, k = 1,m = 50):
    """
    ...
    """
    S = compute_matchmap_similarity_matrix(image_outputs, audio_outputs)
...
def computeMatchmap(I, A):
    ...
    matchmap = F.cosine_similarity(I,A,0)
    return matchmap
...
def sampled_margin_rank_loss(image_outputs, audio_outputs, Imlabel, Audiolabel, margin=1.): #loss function
    """
    ...
    for i in range(n):
        ...
        anchorsim = computeMatchmap(image_outputs[i], audio_outputs[i]) # anchor
        Iimpsim = computeMatchmap(image_outputs[I_imp_ind], audio_outputs[i]) 
        Aimpsim = computeMatchmap(image_outputs[i], audio_outputs[A_imp_ind]) #original image and impostor caption
...
def compute_matchmap_similarity_matrix(image_outputs, audio_outputs):
    """
    ...
    for image_idx in range(n_images): 
            for audio_idx in range(n_audios):
                S[image_idx, audio_idx] = computeMatchmap(image_outputs[image_idx], audio_outputs[audio_idx])
    return S
